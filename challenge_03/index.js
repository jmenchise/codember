...
const validatePass = (keyPolicy, pass) => {
   const [minMaxCount, key] = keyPolicy.split(' ');
   const [minCount, maxCount] = minMaxCount.split('-').map(n => Number(n));
   let count = 0;
   for (letter of pass) {
      if (key === letter) {
         count++;
...
const validkeysCount = (keyList) => {
   let validCount = 0;
   for (item of keyList) {
      const [keyPolicy, pass] = item.split(':');
      const cleanPass = pass.trim();
      if (validatePass(keyPolicy, cleanPass)) {
         validCount++;
      }
   }
   return `Cantidad de claves válidas: ${validCount}`;
}

console.log(validkeysCount(encrypted_policies_list));

const invalidKeysList = encrypted_policies_list.filter(item => {
   const [keyPolicy, pass] = item.split(':');
   const cleanPass = pass.trim();
   if (!validatePass(keyPolicy, cleanPass)) {
      return item;
...
console.log('Clave no válida número 42:', invalidKeysList[41]);
